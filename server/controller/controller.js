            ...
            if (result) {
                ...
                Userdb.findOne({ email: email }, async (err, result) => {
                    if (result) {
                        ...
                        const link = `http://127.0.0.1:3005/api/verifyemail/${user._id}/${token}`
                        console.log(link)
                        ...
                        res.status(200).json({ "status": "succes", "message": "Verification Email sent to your mail id", "info": info })
